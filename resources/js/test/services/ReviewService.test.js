...
(() => {
    ...
    describe("ReviewService", () => {
        ...
        it('should call the recommendation-api while requesting recommendations for a certain review', () => {
            let dbrefer = 'toe-2006-newsentimentality';
            httpBackend.whenGET('/review/' + dbrefer + '/recommendations').respond(getReviews());
            ReviewService.getRecommendedReviews(dbrefer).then((reviews) => {
                expect(reviews.data.length).toBe(2);
            });
            httpBackend.flush();
        });

        it('should call the recommendation-api while requesting recommendations in general', () => {
            httpBackend.whenGET('/review/recommendations').respond(getReviews());
            ReviewService.getRecommendedReviews().then((reviews) => {
                expect(reviews.data.length).toBe(2);
            });
            httpBackend.flush();
        });
